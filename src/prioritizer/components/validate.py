...
from prioritizer.components.evaluation import ModelEvaluator
from prioritizer.components.timechop import Timechop

from prioritizer.utils.conf import convert_str_to_relativedelta
from prioritizer.utils.dbutil import AVAILABLE_TIEBREAKERS
from prioritizer.validation_primitives import string_is_tablesafe
...
class FeatureGroupDefinitionValidator(Validator):
    ...
    def _run(self, feature_group_definition, feature_aggregation_config):
        ...
        available_subsetters = (
            FeatureGroupCreator.subsetters
        )
...
class FeatureGroupStrategyValidator(Validator):
    def _run(self, feature_group_strategies):
        ...
        available_strategies = {
            key
            for key in FeatureGroupMixer.strategy_lookup.keys()
        }
...
class PredictionConfigValidator(Validator):
    def _run(self, prediction_config):
        ...
        if rank_tiebreaker and rank_tiebreaker not in AVAILABLE_TIEBREAKERS:
            raise ValueError(
                "Section: prediction - "
                f"given tiebreaker must be in {AVAILABLE_TIEBREAKERS}"
            )
